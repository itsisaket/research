...
public function actionIndex()
{
    ...
    // 👉 ดึงสถานะผู้ใช้จากระบบ auth
    $user    = Yii::$app->user->identity;
    $isGuest = Yii::$app->user->isGuest;

    // ======================================
    // 1) เช็ก "ครั้งแรกเท่านั้น" ก่อนเลย
    // ======================================
    ...
    if ($loginFlag != 9) {
        ...
        // ถ้ายังไม่ได้ล็อกอิน → ส่งไปหน้า login
        if ($isGuest) {
            return $this->redirect(['site/login']);
        }

        // ถ้าล็อกอินอยู่แล้ว แต่เพิ่งเข้ามาครั้งแรก
        // จะให้วนกลับมาที่ index อีกรอบก็ได้
        return $this->redirect(['site/index']);
    }

    // ======================================
    // 2) มาถึงตรงนี้แปลว่า "เคยเช็กแล้ว"
    //    ค่อยเตรียมตัวแปรไว้ส่งให้ view
    // ======================================
    $displayName  = null;
    $displayEmail = null;

    if (!$isGuest && $user) {
        // ลองดึงจาก session เผื่อมีโปรไฟล์จาก HRM
        $hrmProfile = $session->get('hrmProfile', []);

        // ชื่อเรียงลำดับความสำคัญ
        $displayName =
            ($user->uname ?? null)                           // tb_user.uname
            ...
            ?: ($user->username ?? null)                     // fallback สุดท้าย
            ?: 'ไม่ระบุชื่อ';

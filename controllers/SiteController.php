...
class SiteController extends Controller
{
    ...
    public function actionMyProfile(): array
    {
        ...
        if (!$s2sOk && empty($profile)) {
            $profile = [
                'username'            => (string)$personalId,                 // ใช้เป็น key เสมอ
                'uname'               => (string)($claims['uname'] ?? ''),    // ใช้โชว์ UI ได้ แต่ไม่เป็น key
        ...
        try {
            $username = (string)$personalId; // ใช้ personal_id เป็น username เสมอ
            if ($username === '') {
                Yii::$app->response->statusCode = 400;
            ...
            // prefix mapping (ถ้าเก็บเป็นตัวเลข)
